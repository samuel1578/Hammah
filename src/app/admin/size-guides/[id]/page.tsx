...
export default function EditSizeGuidePage() {
  ...
  const fetchGuide = useCallback(async () => {
    const res = await fetch(`/api/admin/size-guides/${guideId}`);
    if (!res.ok) {
      setError("Failed to load size guide.");
      setLoading(false);
      return;
    }
    const data: SizeGuide = await res.json();
    setName(data.name);
    ...
    data.size_guide_rows.forEach((r) => Object.keys(r.measurements ?? {}).forEach((k) => keys.add(k)));
  ...
  async function handleSave() {
    ...
    if (!res.ok) {
      ...
      setError(data.error || "Failed to save size guide.");
      setSaving(false);
      return;
    }
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-serif italic text-foreground">Edit Size Guide</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Update measurements and settings.
          </p>
        </div>
      </div>
