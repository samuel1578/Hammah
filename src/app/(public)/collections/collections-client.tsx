...
export function CollectionsClient({ collections }: CollectionsClientProps) {
  const c001 = collections.find((c) => c.slug === "collection-001");
              ...
              <div>
                ...
                <Reveal delay={0.25}>
                  <p className="mt-4 max-w-md type-body text-muted-foreground">
                    {c001.description ?? "Eight trouser designs form the first orderable Hammah collection."}
                  </p>
                </Reveal>
                <Reveal delay={0.3}>
                  <div className="mt-6">
                    <Link
                      href={`/collections/${c001.slug}`}
